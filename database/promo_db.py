...
def get_promo(code: str, product_id: int | None = None):
    """Return active promo. Global promos have product_id NULL; product promos match only target product."""
    code = code.strip().upper()
    conn = get_conn()
    cursor = conn.cursor()
...
def get_all_promos():
    ...
    cursor.execute(
        """
        SELECT pc.code, pc.discount_percent, pc.is_active, pc.created_at, pc.product_id, p.name AS product_name
        FROM promo_codes pc
